...
Option Explicit
'============================================================
' MS2 コアマクロ v7fix-200（2026-08-31）
'  ベース: ATR_MS2_V08021.xlsm（v6 / 最新保存）＋ fix_v7 の全修正
'
'  ■ v7fix-200 での追加修正
'   F-1  fix_v7 に混入していた文字列リテラルの構文エラー8箇所を修正
'        （行末の余分な " によりモジュール全体がコンパイル不能だった）
'   F-2  ソースを Shift-JIS(CP932) で格納。日本語リテラルの文字化けを解消
'   F-3  ★対応銘柄数を 200 に拡張（STOCK_MAX/STOCK_LAST/DATA_LAST/HIST_LAST を連動）
'   F-4  旧350行レイアウトの残骸を 400行目まで掃除（DATA_CLEAN / HIST_CLEAN）
'   F-5  StockList 参照の固定 $2:$400 を銘柄数連動に変更
'   F-6  TRADE_MS2 見出しの「(100株)」表記を「(円)」に統一（実体は H13 換算）
'   F-7  MS2_Version（バージョン確認）を追加。操作パネルにもボタンを追加
'
'  ■ v6 からの変更点（fix_v7 由来 / 不具合ID は診断レポートに対応）
'  ------------------------------------------------------------
...
'★200銘柄対応（v7fix-200）: ここを変えれば対応銘柄数を増減できる
Private Const STOCK_MAX    As Long = 200                       ' 扱う銘柄数
Private Const STOCK_LAST   As Long = STOCK_MAX + 1             ' StockList_MS2 の最終行(1行目=見出し)
Private Const DATA_LAST    As Long = DATA_FIRST + STOCK_MAX - 1 ' DATA_MS2 の最終行
Private Const DATA_CLEAN   As Long = 400                       ' 旧レイアウトの残骸を掃除する下限行
Private Const HIST_CLEAN   As Long = 400                       ' ATRHIST を掃除する下限行
...
Private Const HIST_LAST    As Long = STOCK_LAST  ' ATRHIST の参照上限（銘柄数に連動）
Private Const PF_INF       As Double = 999  ' PF/RR の「∞」を表す数値
Private Const MS2_VER      As String = "MS2_Core v7fix-200 (2026-08-31)"
...
Private Function NameFormula(ByVal rowNo As Long) As String
    NameFormula = "=IFERROR(INDEX(StockList_MS2!$B$2:$B$" & STOCK_LAST & _
                  ",MATCH($B" & rowNo & ",StockList_MS2!$A$2:$A$" & STOCK_LAST & ",0)),"""")"
End Function
...
Sub MS2_Install_Formulas()
    ...
    '★余った行に古い数式が残っていたら消す
    If lastRow < DATA_CLEAN Then
        ws.Range("M" & (lastRow + 1) & ":AD" & DATA_CLEAN).ClearContents
    End If
...
Sub MS2_Update_StockList_To_DATA()
    ...
    If lastL > STOCK_LAST Then lastL = STOCK_LAST          '★200銘柄まで
    For s = 2 To lastL
    ...
    If DATA_FIRST + n <= DATA_CLEAN Then
        wsD.Range("A" & (DATA_FIRST + n) & ":AD" & DATA_CLEAN).ClearContents
    End If
    Application.ScreenUpdating = True
...
Sub MS2_Set_RssMarket_Formulas()
    ...
    If lastRow < DATA_CLEAN Then
        ws.Range("E" & (lastRow + 1) & ":L" & DATA_CLEAN).ClearContents
    End If
    Application.ScreenUpdating = True
...
Sub MS2_Reset_ATR_History()
    ...
    wsH.Range("A2:I" & HIST_CLEAN).ClearContents
    gBucket = -1
...
Sub MS2_Show_Log()
    ...
    If gSampling And GetTimerCell(CELL_TICK) <= 0 Then
        warn = vbCrLf & vbCrLf & "【注意】 「稼働中」なのに予約がありません。" & vbCrLf & _
               "　「★タイマー全解除」→「ATR収集_開始」で復旧してください。"
    End If
    If lastUpd > 0 Then
        If Int(lastUpd) <> Int(CDbl(Date)) Then
            warn = warn & vbCrLf & vbCrLf & "【注意】 ATR履歴が古い（" & Format(CDate(lastUpd), "yyyy/mm/dd hh:nn") & "）。" & vbCrLf & _
...
Sub MS2_Fix_TRADE_Header()
    ...
    'サマリー（★同日決済のみを集計）
    ws.Range("R1").Value = "◆ 売買単位換算サマリー（同日決済のみ）"
...
' 全自動
'============================================================
Public Sub MS2_Version()
    MsgBox MS2_VER & vbCrLf & vbCrLf & _
           "対応銘柄数: " & STOCK_MAX & " 銘柄" & vbCrLf & _
           "StockList_MS2 : A2:B" & STOCK_LAST & vbCrLf & _
           "DATA_MS2      : 3行目～" & DATA_LAST & "行目" & vbCrLf & _
           "ATRHIST_MS2   : A2:I" & HIST_LAST & vbCrLf & _
           "TRADE_MS2     : 2行目～" & TRADE_LAST & "行目", vbInformation, "MS2"
End Sub
