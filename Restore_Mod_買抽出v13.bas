...
Option Explicit
'=============================================================================
' 打ち出の無限こづち　候補抽出モジュール　v13 (2026/06/17)
'=============================================================================
...
Public Const BUY_REQUIRE_MOMENTUM_COUNT As Long = 3
...
Public Const BUY_REQUIRE_PERFECT_ORDER As Boolean = True
Public Const BUY_REQUIRE_VOL_BURST     As Boolean = False
Public Const BUY_REQUIRE_ADX           As Boolean = False
Public Const BUY_REQUIRE_HIST_EXPAND   As Boolean = False
...
Public Sub 買抽出v13ボタン設置()
    ...
    If ws Is Nothing Then
        MsgBox "「買抽出v13」シートが見つかりません。", vbExclamation
        Exit Sub
    End If
    Dim shp As Shape
    ...
    With btn1
        .name = "btn高精度抽出"
        .TextFrame.Characters.Text = "★高精度抽出(12以上)"
    ...
    End With
    MsgBox "ボタンを再設置しました！", vbInformation, "完了"
End Sub
...
Public Sub 株価抽出()
    ...
    If (h >= 9 And h < 15) Or (h = 15 And m <= 30) Then
        Call 抽出実行_RSS(12, True, "高精度抽出[RSS現在値](スコア12以上)")
    Else
        Call 抽出実行(12, True, "高精度抽出(スコア12以上・全フィルター必須)")
    End If
End Sub

Public Sub 買い抽出_スコア4()
    抽出実行 11, True, "買い抽出(スコア11以上・中程度)"
End Sub

Public Sub 買い抽出_スコア4_緩和()
    抽出実行 11, False, "買い抽出(スコア11以上・緩和)"
End Sub
...
Private Sub 抽出実行(ByVal minScore As Long, _
    ...
    If closeWs Is Nothing Or extWs Is Nothing Then
        MsgBox "「終値」または「買抽出v13」シートが見つかりません。", vbExclamation
        Exit Sub
    End If
    ...
    For i = STOCK_DATA_START To lastRow
        ...
        If totalScore >= minScore Then
            Select Case topixMode
                Case "LARGE"
                    If emaTrend = "パーフェクト▲" And osime <> "なし" And osime <> "" Then canExtract = True
                Case "MID"
                    If emaTrend <> "下降トレンド▼" Then canExtract = True
                Case Else
                    If totalScore >= minScore + 2 Then canExtract = True
            End Select
        End If
        If canExtract And momCount < 2 Then canExtract = False
        If doMark And canExtract Then
            If momCount < BUY_REQUIRE_MOMENTUM_COUNT Then canExtract = False
        End If
        If doMark And canExtract Then
        ...
        If doMark And canExtract And BUY_REQUIRE_OSIME Then
            If osime <> "25日線押し目●" And osime <> "5日線押し目○" Then
                canExtract = False
            End If
        End If
...
NextStock:
    ...
    If outRow > EXT_DATA_ROW + 1 Then
        extWs.Range(extWs.Cells(EXT_DATA_ROW, 1), _
            ...
            Key1:=extWs.Cells(EXT_DATA_ROW, 17), Order1:=xlDescending, Header:=xlNo
    End If
    On Error Resume Next
    ...
    Next chkRow
    Dim msg As String
    msg = "【打ち出の無限こづち v13 押し目必須・ダマシ強化】" & vbCrLf & _
          ...
          "(抽出基準: スコア " & minScore & " 点以上)" & vbCrLf & vbCrLf & _
          "--- v13変更点 ---" & vbCrLf & _
    ...
    If doMark Then
        msg = msg & vbCrLf & vbCrLf & "--- 高精度フィルター通過数 ---" & vbCrLf & _
              ...
              "対TOPIX相対強度:    " & cntRS
    End If
    MsgBox msg, vbInformation, "抽出完了 (v13)"
End Sub

Private Sub 抽出実行_RSS(ByVal minScore As Long, _
                          ByVal doMark As Boolean, _
                          ByVal titleText As String)
    Call 抽出実行(minScore, doMark, titleText & "[RSS]")
End Sub
