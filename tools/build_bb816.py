...
■ V815 の BB判定から変えたところ（すべて実測が根拠）
  ...
  ※ 寄り付きが逆指値より下に飛んだら寄値で約定する（＝逆指値では止まらない）前提で計算
      BB816     通し +85.5%  開始日13通りの中央値 +70.9%（最小+43.8%）  最大DD -23.5%
      現行スコア  中央値 +34.9%（最小 -2.4%）
      無選別     中央値 +51.4%（乱数20通り・最小+7.9%）  最大DD -14.5%
      1トレード最悪 -32.2%（窓開け決済が全体の10.5%、平均 -12.83%）
...
GUIDE = [
    ...
    ("N", "          （窓を開けて飛んだ場合は -8% では止まりません。実測で全体の10.5%が該当し、平均 -12.8%）"),
    ("L", "手仕舞い  10営業日たったら損益に関わらず引けで売る"),
    ("L", "置かない  利確目標・トレーリングストップ・地合いフィルタ（いずれも検証で成績を下げた）"),
    ("N", "          固定利確+6%は中央値 -25.2%（負ける）。+6%到達でトレールに切り替える案も、"),
    ("N", "          最良のトレール幅-8%で中央値+50.9%どまりで、トレールなしの+63.9%に届きませんでした。"),
    ("", ""),
    ("S", "■ 検証結果（枠8・1日3銘柄・10日保有・損切-8%・250営業日）"),
    ("N", "寄り付きが逆指値より下に飛んだ場合は寄値で約定する（窓開け）ものとして計算しています。"),
    ("L", "BB816            通し +85.5%   開始日13通りの中央値 +70.9%（最小+43.8%）  最大DD -23.5%   取引248回"),
    ("L", "往復コスト0.2%込み  中央値 +63.9%（最小+38.7%）"),
    ("L", "V815の現行スコア  中央値 +34.9%（最小 -2.4%）  最大DD -14.4%"),
    ("L", "何も選ばず買う     中央値 +51.4%（乱数20通り）  最大DD -14.5%"),
    ("N", "無選別が BB816 を上回ったのは20通り中2通り（10%）。勝ってはいますが差は大きくありません。"),
    ("L", "手仕舞いの内訳    期日149件(60.1%) 平均+10.00% / 損切り73件(29.4%) -8.00% / 窓開け26件(10.5%) -12.83%"),
    ("L", "1トレードの最悪   -32.2%（窓開けで逆指値を大きく下回った例）。枠8なら総資金の -4.0%"),
    ("N", "★ 逆指値は「-8%で必ず止まる」ものではありません。悪材料で寄り付きが飛べば、その値段で約定します。"),
    ("", ""),
    ...
    ("N", "・その損切りも、寄り付きが飛べば効きません（実測で10.5%が窓開け決済、平均-12.8%、最悪-32.2%）。"),
