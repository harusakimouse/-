...
Public Sub 平均足_記録シート作成()
    ...
    keepLot = HA_L_Num(ws.Cells(2, 4).Value, 0.05)
    If keepLot <= 0 Or keepLot > 1 Then keepLot = 0.05
    ...
    ws.Cells(3, 13).Value = "今の指示"
    With ws.Range("N3:Q3")
        .Merge
        .Formula = "=IF(H3<=$J$2,""今月はもう建てない（月間ストップ）"",IF($J$3>=3,""3連敗中：株数を半分にする"",IF(L3>=5,""枠が埋まっています"",""通常どおり。上位から買ってよい"")))"
    ...
    hd = Array("買日", "コード", "銘柄名", "買値", "株数", "投資額", "損切値", "利確値", _
               "期限日", "売日", "売値", "損益(円)", "損益率", "結果", "資金比", "メモ", "連敗")
    Dim j As Long
    ...
    For r = TOP_ROW To MAX_ROW
        ...
        ws.Cells(r, 9).Formula = "=IF(A" & r & "="""","""",WORKDAY(A" & r & ",7))"
    ...
    '--- 連敗の列は使わないので薄く ---
    ws.Columns("Q").Font.Color = RGB(200, 200, 200)
    ...
    ws.Cells(g + 4, 1).Value = "4. 「期限日」を過ぎた保有中の玉は、その日の引けで成行手仕舞い。"
    ...
    ws.Cells(g + 9, 1).Value = "・10回中4〜5回は負けます。それが設計です。負けは家賃だと思ってください。"
    ws.Cells(g + 10, 1).Value = "・最初の3ヶ月は「1銘柄の割合」を5%のままで。1回の負けは資金の0.4%です。"
    ws.Cells(g + 11, 1).Value = "・慣れたら 10% → 15% → 20% と上げる。20%で 年+32.9%・最大下落6.8% が検証値です。"
    With ws.Range("A" & g & ":A" & (g + 11))
        .Font.Name = "Meiryo UI": .Font.Size = 10
    End With
    ...
    MsgBox "「" & SH & "」シートを作りました。" & vbCrLf & vbCrLf & _
           ...
           "まずは B2 の資金と、D2 の1銘柄の割合（最初は5%）を確認してください。", vbInformation
End Sub
