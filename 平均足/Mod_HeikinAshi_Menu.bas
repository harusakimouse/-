...
Public Sub 平均足_メニュー作成()
    ...
    ws.Cells(r + 9, 2).Value = "※朝いちばんに⑩データチェックを押して「OK」を確認してから買うこと（RSSが切れた日の事故防止）"
    With ws.Range(ws.Cells(r, 2), ws.Cells(r + 9, 2))
    ...
    ws.Cells(r + 9, 2).Font.Color = RGB(192, 0, 0)
    ws.Cells(r + 9, 2).Font.Bold = True
